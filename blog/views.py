...
import markdown
#markdown格式的文本渲染成标准的 HTML 文档是一个复杂的工作
from comments.forms import CommentsForm#引入评论表单
from django.views.generic import ListView, DetailView


class IndexView(ListView):
    model = Article#告诉 Django 我要获取的模型是 Article。
    ...
    def get_context_data(self, **kwargs):#需要传递的模板变量字典是通过 get_context_data 获得的，
        """
...
class ArticleDetailView(DetailView):
    ...
    def get_object(self, queryset=None):
        # 覆写 get_object 方法的目的是因为需要对 context值进行渲染
        ...
        article.context = md.convert(article.context)
        article.toc = md.toc
        #动态添加属性（添加toc属性）
        return article
