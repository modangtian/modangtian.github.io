...
def article_comment(request, article_pk):
    ...
    article = get_object_or_404(Article, pk=article_pk)#在这有遇到个坑，原因是写代码没认真，少写pk
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # commit=False 的作用是仅仅利用表单的数据生成 Comment 模型类的实例，但还不保存评论
...
'''这个评论视图相比之前的一些视图复杂了很多，主要是处理评论的过程更加复杂。
具体过程在代码中已有详细注释，这里仅就视图中出现了一些新的知识点进行讲解。
